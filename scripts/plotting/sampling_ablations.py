...
from data_locations import training_ablations as data_locations
...
root_dir, fig_path, experiment_root_dir, seeds, expert_root, expert_perf_files, expert_perf_file_main_task_i = \
    plot_common.get_path_defaults(fig_name="sampling_ablations", task_inds=[0])  # stack only
...
num_datas = ["expert_train_for_qp_final_ex_prop"]  # keys in dictionary
# num_datas_labels = ["LfGP Scheduler", r"Expert Sampling for Q/$\pi$", "Final Example Sampling Rate"]
num_datas_labels = ["Expert Sampling"]
valid_algos = ['lfgp_wrs', 'multitask_bc', 'dac', 'bc']
# algo_dir_names = [
#     ['lfgp_wrs', 'lfgp_wrs_no_hc', 'lfgp_learned', 'lfgp_no_sched'],
#     ['lfgp_wrs', 'lfgp_wrs_wo_qp', 'dac', 'dac_wo_qp'],
#     ['lfgp_wrs', 'lfgp_wrs_eq_prop', 'dac', 'dac_eq_prop'],
# ]
algo_dir_names = [
    # ['lfgp_wrs', 'lfgp_wrs_no_hc', 'lfgp_learned', 'lfgp_no_sched'],
    ['lfgp_wrs', 'lfgp_wrs_wo_qp', 'lfgp_wrs_eq_prop', 'dac', 'dac_wo_qp', 'dac_eq_prop'],
]
algo_titles = [
    # ['LfGP (WRS + Handcraft)', 'LfGP (WRS only)', 'LfGP (Learned)', 'LfGP (No Scheduler)'],
    # ['WRS + Handcraft', 'WRS only', 'Learned', 'No Scheduler'],
    # ['LfGP (Original)', r'LfGP (No Q \& $\pi$ sampling from $\mathcal{B}^{E}$)', r'LfGP (equal sampling for $D$)', 
    #  'DAC (Original)', r'DAC (No Q \& $\pi$ sampling from $\mathcal{B}^{E}$)', r'DAC (equal sampling for $D$)'],
    ['LfGP', r'LfGP ($\mathcal{B}^{E}$ for $D$ only)', r'LfGP (No $(s_T,a_T)$ bias)', 
     'DAC', r'DAC ($\mathcal{B}^{E}$ for $D$ only)', r'DAC (No $(s_T,a_T)$ bias)'],
]
algo_cmap_i = [
    [0, 4, 5, 2, 6, 7]
]
...
# fig
# scheduler: 4 options with DIFFERENT colours from algorithm colours (or possibly all blue, but diff line types?)
# expert_train_for_qp: with and without for LfGP and DAC (same colour for algo, diff line types for with/without qp) 
# final_ex_prop: same as above

# TODO the labelling has to be done completely differently, can probably do it more cleverly to not rewrite things but
# still differentiate between LfGP and DAC
# TODO the curves are completely wrong for some reason
# TODO need to switch to different colour changes or keeping colours from main plot but changing line types
...
if half_width:
    plot_size = [2, 2.4]
    algo_titles = [
        ['LfGP', r'\noindent LfGP ($\mathcal{B}^{E}$ \newline for $D$ only)', r'\noindent LfGP (No \newline $(s_T,\boldsymbol{0})$ bias)', 
        'DAC', r'\noindent DAC ($\mathcal{B}^{E}$ \newline for $D$ only)', r'\noindent DAC (No \newline $(s_T,\boldsymbol{0})$ bias)'],
    ]
#####################################################################################################################
    ...
    for num_data_i, num_data in enumerate(num_datas):
        for algo_i, algo in enumerate(algo_dir_names[num_data_i]):
            ...
            for seed in seeds:
                ...
                if algo in multitask_algos or 'lfgp' in algo:
                    suc_fixed = []
                    ret_fixed = []
...
for fig, fig_name in zip([s_fig, r_fig], ['s_fig.pdf', 'r_fig.pdf']):
    ...
    if half_width:
        fig.legend(fancybox=True, shadow=True, fontsize=font_size-7.5, loc="center right", ncol=1, bbox_to_anchor=(1.575, 0.475))
    else:
