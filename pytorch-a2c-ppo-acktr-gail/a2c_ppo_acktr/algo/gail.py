...
class Discriminator(nn.Module):
    ...
    def update(self, expert_loader, rollouts, obsfilt=None):
        ...
        for expert_batch, policy_batch in zip(expert_loader,
            ...
            # print(expert_state.shape, expert_action.shape)
            expert_d = self.trunk(
...
class LfGPExpertDataset(torch.utils.data.Dataset):
    def __init__(self, file_name):
        self.trajectories = torch.load(file_name)
        self.length = len(self.trajectories["states"])
            
    def __len__(self):
        return self.length

    def __getitem__(self, i):
        return self.trajectories['states'][i], self.trajectories[
            'actions'][i]
