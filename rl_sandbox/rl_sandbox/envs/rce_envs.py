...
"""Environments for experiments with RCE. Modified from original version from RCE paper at
...
# silence annoying d4rl warnings
...
# from rl_sandbox.envs.hand_dapg_dp_envs import RelocateEnvV0DP, RelocateEnvV0NoArmJointPosObs, RelocateEnvV0NoArmJointPosObsDP
import rl_sandbox.envs.hand_dapg_dp_envs as hand_dapg_dp_envs
...
@gin.configurable
# def load_env(env_name, gym_env=False, max_episode_steps=None, aux_tasks=('reach')):
def load_env(env_name, gym_env=False, max_episode_steps=None, grip_pos_in_env=False, vel_in_env=False,
             hand_dapg_dp_kwargs={}):
  ...
  """
  import rl_sandbox.envs.rce_multitask_envs as rme

  # sawyer env handling, including reach aux
  reach_aux = False
  random_reach_aux = False
  if env_name not in ['sawyer_reach', 'sawyer_air_reach']:
    random_grasp_aux = 'random_grasp' in env_name
    grasp_aux = 'grasp' in env_name
    reach_aux = 'reach' in env_name
    random_reach_aux = 'random_reach' in env_name
    if 'random_grasp' in env_name:
      env_name = env_name.split('_random_grasp')[0]
    elif 'grasp' in env_name:
      env_name = env_name.split('_grasp')[0]
    elif 'random' in env_name:
      env_name = env_name.split('_random_reach')[0]
    elif 'reach' in env_name:
      env_name = env_name.split('_reach')[0]

  if env_name == 'sawyer_reach':
    gym_env = SawyerReach()
    max_episode_steps = 50
  elif env_name == 'sawyer_air_reach':
    gym_env = SawyerAirReach()
    max_episode_steps = 50
  elif env_name == 'sawyer_push':
    gym_env = SawyerPush()
    max_episode_steps = 150
    random_reach_env_noise = (0.1, 0.1, 0.02)
    random_grasp_env_noise = (0.1, 0.1, 0.025)
    # random_reach_env_noise = (0.6, 0.6, 0.)
  elif env_name == 'sawyer_lift':
    gym_env = SawyerLift()
    max_episode_steps = 150
    random_reach_env_noise = (0.025, 0.025, 0.02)
    random_grasp_env_noise = (0.025, 0.025, 0.025)
  elif env_name == 'sawyer_drawer_open':
    gym_env = SawyerDrawerOpen()
    max_episode_steps = 150
    random_reach_env_noise = (0.01, 0.03, 0.02)  # first dim is for gripper only, second is drawer + gripper together
    random_grasp_env_noise = (0.01, 0.03, 0.02)
    ...
    max_episode_steps = 150
    random_reach_env_noise = (0.01, 0.01, 0.02)
    random_grasp_env_noise = (0.01, 0.01, 0.025)
  elif env_name == 'sawyer_bin_picking':
    gym_env = SawyerBinPicking()
    max_episode_steps = 150
    random_reach_env_noise = (0.04, 0.04, 0.02)
    random_grasp_env_noise = (0.04, 0.04, 0.025)
  elif env_name == "relocate-human-v0-dp":
    gym_env = hand_dapg_dp_envs.RelocateEnvV0DP(**hand_dapg_dp_kwargs)
    max_episode_steps = int(200 / gym_env.frame_skip * 5)
  elif env_name == "door-human-v0-dp":
    gym_env = hand_dapg_dp_envs.DoorEnvV0DP(**hand_dapg_dp_kwargs)
    max_episode_steps = int(200 / gym_env.frame_skip * 5)
  elif env_name == "hammer-human-v0-dp":
    gym_env = hand_dapg_dp_envs.HammerEnvV0DP(**hand_dapg_dp_kwargs)
    max_episode_steps = int(200 / gym_env.frame_skip * 5)
  elif env_name == "relocate-human-v0-najp":
    gym_env = hand_dapg_dp_envs.RelocateEnvV0NoArmJointPosObs(include_vel=vel_in_env)
    max_episode_steps = 200
  elif env_name == "relocate-human-v0-najp-dp":
    gym_env = hand_dapg_dp_envs.RelocateEnvV0NoArmJointPosObsDP(**hand_dapg_dp_kwargs)
    max_episode_steps = int(200 / gym_env.frame_skip * 5)
  else:
  ...
  if 'sawyer' in env_name:
    if random_reach_aux:
      gym_env = rme.sawyer_aux_reach_gen(gym_env, random_reach_env_noise)()
      env_name += "_random_reach"
    elif reach_aux:
      gym_env = rme.sawyer_aux_reach_gen(gym_env)()
      env_name += "_reach"
    elif random_grasp_aux:
      gym_env = rme.sawyer_aux_reach_gen(gym_env, random_grasp_env_noise, grasp=True)()
      env_name += "_random_grasp"
    elif grasp_aux:
      gym_env = rme.sawyer_aux_reach_gen(gym_env, grasp=True)()
      env_name += "_grasp"

    # make env a grip pos env, optionally
    if grip_pos_in_env or vel_in_env:
      # gym_env = get_env_with_grippos(gym_env)()
      gym_env = get_env_with_more_obs(gym_env, grip_pos=grip_pos_in_env, ee_vel=vel_in_env, obj_vel=vel_in_env)()

  else:
    # non-sawyer reach aux handling
    assert not 'random' in env_name, "No randomization implemented for hand_dapg envs."
    if reach_aux:
      gym_env = rme.hand_dapg_reach_gen(gym_env, mode='reach')()
      env_name += "_reach"
    elif grasp_aux:
      gym_env = rme.hand_dapg_reach_gen(gym_env, mode='grasp')()
      env_name += "_grasp"
    elif not reach_aux and not grasp_aux and 'najp' in env_name and not 'dp' in env_name:
      # need to handle najp env end states same as how we handle reach and grasp, since expert data is different now
      gym_env = rme.hand_dapg_reach_gen(gym_env, mode='final')()

  if gym_env:
    gym_env = TimeLimit(gym_env, max_episode_steps=max_episode_steps)
    return gym_env
  else:
...
# def get_env_with_grippos(env):
def get_env_with_more_obs(env, grip_pos=True, ee_vel=True, obj_vel=True):
  GripPosEnvClass = type('GripPosEnvClass', (type(env,),), {})

  def new_get_obs(self):
    obs = super(GripPosEnvClass, self)._get_obs()
    extra = []

    if ee_vel:
      extra.append(self.data.get_body_xvelp('hand'))
    if obj_vel:
      if isinstance(env, SawyerDrawerOpen) or isinstance(env, SawyerDrawerClose):
        extra.append(self.data.get_body_xvelp('drawer'))
      else:
        extra.append(self.data.get_body_xvelp('obj'))
    if grip_pos:
      # obs = np.concatenate([obs, self.data.qpos[7:9]])
      extra.append(self.data.qpos[7:9])

    obs = np.concatenate([obs, *extra])
    return obs

  GripPosEnvClass._get_obs = new_get_obs

  # fix observation space as well
  ...
  def new_init(self, **kwargs):
    super(GripPosEnvClass, self).__init__(**kwargs)
    self.obs_dim = new_obs_dim
    high = np.inf*np.ones(new_obs_dim)
    low = -high
    self.observation_space = spaces.Box(low, high, dtype=np.float32)

  GripPosEnvClass.__init__ = new_init

  return GripPosEnvClass
...
def get_data(env, env_name, num_expert_obs=200, terminal_offset=50):
  ...
  """
  if env_name in ['hammer-human-v0', 'door-human-v0', 'relocate-human-v0', 'pen-human-v0']:
    ...
    if dataset['observations'].shape[0] < num_expert_obs:
      indices = np.random.choice(
          dataset['observations'].shape[0], size=num_expert_obs, replace=True)
    else:
      indices = np.random.choice(
...
class SawyerReach(sawyer_xyz.SawyerReachPushPickPlaceEnv):
  ...
  def step(self, action):
    obs = self._get_obs()
    d_before = np.linalg.norm(obs[:3] - obs[3:6])
    s, r, done, info = super(SawyerReach, self).step(action)
    d_after = np.linalg.norm(s[:3] - s[3:6])
    r = d_before - d_after
    done = False
...
class SawyerAirReach(SawyerReach):
  """An even simpler reaching environment."""

  def __init__(self):
    super(SawyerAirReach, self).__init__()

    self._goal = np.array([0.0, 0.6, 0.15])

  def step(self, action):
    obs = self._get_obs()
    d_before = np.linalg.norm(obs[:3] - self._goal)
    s, r, done, info = super(SawyerAirReach, self).step(action)
  ...
  def _get_expert_obs(self):
    ...
    # an image for SawyerReachImage.
    obs = self._get_obs()
    self.data.set_mocap_pos('mocap', self._goal)
    self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)
    # Hide the markers, which get reset after every simulation step.
...
class SawyerPush(sawyer_xyz.SawyerReachPushPickPlaceEnv):
  ...
  def step(self, action):
    obs = self._get_obs()
    d_before = np.linalg.norm(obs[3:6] - self._goal)
    s, _, done, info = super(SawyerPush, self).step(action)
    d_after = np.linalg.norm(s[3:6] - self._goal)
    r = d_before - d_after
    done = False
    return s, r, done, info
...
class SawyerLift(sawyer_xyz.SawyerReachPushPickPlaceEnv):
  ...
  def step(self, action):
    ...
    # print(f"grip poss: {self.data.qpos[7:9]}")
  ...
  def _get_expert_obs(self, reset=True):
    ...
    # the original RCE version had puck x,y position fixed, but this can hinder learning...adding a small amount of noise
    puck_pos[:2] = np.random.uniform(puck_pos[:2] - .02, puck_pos[:2] + .02)

    # in the original RCE version, puck was set to middle of gripper teeth...which isn't possible to achieve
    # without "flipping" the puck up. Moving the gripper higher to make more realistic.
    ee_pos = copy.deepcopy(puck_pos)
    ee_pos[-1] += .035

    self.data.set_mocap_pos('mocap', ee_pos)
    self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)
    ...
    # add in grasping, always resetting puck position so it doesn't fall while teeth are closing
    for _ in range(10):
      self.do_simulation([1, -1], self.frame_skip)
      self._set_obj_xyz_quat(puck_pos, 0.0)
...
class SawyerBoxClose(sawyer_xyz.SawyerBoxCloseEnv):
  ...
  def _get_expert_obs(self):
    ...
    self.data.set_mocap_pos('mocap', obs[6:9])
    self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)
    return self._get_obs()
...
class SawyerBinPicking(sawyer_xyz.SawyerBinPickingEnv):
  ...
  def _get_expert_obs(self):
    ...
    goal_pos = np.random.uniform(
        ...
        low=np.array([0.06, 0.66, 0.046]), high=np.array([0.18, 0.74, 0.046]))
    self._set_obj_xyz_quat(goal_pos, self.obj_init_angle)
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)

    # allow ee to just float somewhere above bring box
    ee_goal_pos = np.random.uniform(
      # low=np.array([0.06, 0.64, 0.046]), high=np.array([0.18, 0.76, 0.046]))
      low=np.array([0.1, 0.68, 0.12]), high=np.array([0.14, 0.72, 0.22]))

    # force a middle ee position to ensure no collision with box prior to reach
    obs = self._get_obs()
    first_ee_pos = copy.deepcopy(ee_goal_pos)
    first_ee_pos[2] = obs[2]
    self.data.set_mocap_pos('mocap', first_ee_pos)
    self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)

    # goal_pos[2] = .1  # so it doesn't hit the box and bounce around
    # self.data.set_mocap_pos('mocap', goal_pos)
    # self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))

    self.data.set_mocap_pos('mocap', ee_goal_pos)
    self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    for _ in range(10):
      self.do_simulation([-1, 1], self.frame_skip)
