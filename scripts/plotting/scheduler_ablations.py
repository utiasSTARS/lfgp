...
root_dir, fig_path, experiment_root_dir, seeds, expert_root, expert_perf_files, expert_perf_file_main_task_i = \
    plot_common.get_path_defaults(fig_name="scheduler_ablations", task_inds=[0])  # stack only
...
# num_datas = ["scheduler", "expert_train_for_qp", "final_ex_prop"]  # keys in dictionary
# num_datas = ["scheduler", "expert_train_for_qp_final_ex_prop"]  # keys in dictionary
num_datas = ["scheduler"]  # keys in dictionary
# num_datas_labels = ["LfGP Scheduler", r"Expert Sampling for Q/$\pi$", "Final Example Sampling Rate"]
num_datas_labels = ["LfGP Scheduler", r"Expert Sampling"]
...
algo_titles = [
    ...
    ['WRS + HC', 'WRS only', 'Learned', 'No Sched.'],
    ...
    for num_data_i, num_data in enumerate(num_datas):
        for algo_i, algo in enumerate(algo_dir_names[num_data_i]):
            ...
            for ax, task_algo_amt_data in zip([s_ax, r_ax], [all_successes[task][num_data][algo], all_returns[task][num_data][algo]]):
                ...
                try:
                    # custom modification for schedulers so colours aren't the same as algos in other plots
                    if num_data == "scheduler" and algo_i > 0:
                        cmap_i = algo_i + 3
                    else:
                        cmap_i = algo_i
                    ...
                    ax.plot(x_vals, mean, label=algo_titles[num_data_i][algo_i],
                        color=cmap(cmap_i), linewidth=linewidth, linestyle=line_style)
                    ax.fill_between(x_vals, mean - num_stds * std, mean + num_stds * std, facecolor=cmap(cmap_i),
                                    alpha=std_alpha)
                except:
                    import ipdb; ipdb.set_trace()
...
for fig, fig_name in zip([s_fig, r_fig], ['s_fig.pdf', 'r_fig.pdf']):
    ...
    if half_width:
        ...
        fig.legend(fancybox=True, shadow=True, fontsize=font_size-5, loc="lower center",
                ncol=int(math.ceil((len(algo_dir_names) + 1))), 
                bbox_to_anchor=(0.45, -0.45))  
    # fig.legend(fancybox=True, shadow=True, fontsize=font_size-5, loc="upper left",ncol=1)
